...
class MessageServicer(message_pb2_grpc.MessageServiceServicer):
    ...
    def __init__(self, connections: list[Connection], auth_service: AuthService) -> None:
        super().__init__()
        self.connections = connections
        self.auth_service = auth_service
    
    def construct_message(self, message: any, type: MessageType, senderId: str) -> ServerMessage:
    ...
    async def StreamMessages(self, request: ClientMessage, context):
        ...
        if request.token is None or request.token == "":
            ...
            yield self.construct_message(type=MessageType.AUTH, message=AuthResponse(message="Unauthorized"), senderId="server")
            return  # Exit early without value
    ...
    async def SendMessage(self, request: ClientMessage, context):
        ...
        for connection in self.connections:
            if (connection.id != tokenData.id):
                ...
                await connection.queue.put(self.construct_message(message=payload, type=messageType, senderId=tokenData.id))

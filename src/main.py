...
@app.post("/refresh")
async def get_access_token_with_refresh_token(request: Request, response: Response, role: str, db: Annotated[Database, Depends(Database.get_db)]) -> Token:
    ...
    access_token = Authentication.create_access_token()
    new_refresh_token = Authentication.create_refresh_token()

    db.removeRefreshToken(refresh_token)
    db.addRefreshToken(new_refresh_token, role)
